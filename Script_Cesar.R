...
server <- function(input, output, session) {
  ...
  output$map <- renderLeaflet({
    ...
    prices_map <- prices %>% filter(Fecha == dates_max)
    departamento <- left_join(departamento, prices_map, by = c("gis/Admin1_UnodcOcha_01012009.shp" = "Departamento"))
    
    labels <- sprintf("<strong>%s</strong><br/>%s IQD (%s)", departamento$Admin1Name, format(departamento$'SMEB common basket', big.mark=","), format(departamento$Fecha, "%b %Y")) %>% lapply(htmltools::HTML)
    pal <- colorNumeric(palette = c("#FFF98C", "#E0C45C", "#CB3B3B", "#85203B"),
                        domain = departamento$'Departamento', na.color = "transparent"
    )
    ...
    map <- leaflet(options = leafletOptions(attributionControl=FALSE)) %>%
      ...
      addPolygons(data = district, group = "Departamento", fill = TRUE, fillOpacity = 0.7, fillColor = ~pal(departamento$'SMEB common basket'),
      ...
      addPolygons(data = governorate, group = "Departamento", fill = FALSE, stroke = TRUE, color = "#58585A", weight = 0.6, opacity = 1) %>%
      addPolygons(data = country, group = "Country", fill = FALSE, stroke = TRUE, color = "#58585A", weight = 1.2, opacity = 1) %>%
      addLegend("bottomright", pal = pal, values = departamento$'Prices',
                title = "Price:",
                labFormat = labelFormat(prefix = "COL "),
                opacity = 1
      ) %>%
      ...
      addLayersControl(overlayGroups = "Departamento")
  })
  
  observe({
    
    prices_map <- prices %>% filter(Date == input$map_date_select)
    district <- left_join(departamento, prices_map, by = c("Admin1Name" = "Departamento"))
    indicators_map <- indicators %>% filter(Date == input$map_date_select)
    district <- left_join(departamento, indicators_map, by = c("Admin1Name" = "Departamento"))
    ...
    if (all(is.na(departamento[[map_indicator_select()]])) == TRUE) {
      return(NULL)
    } 
    
    if (input$map_indicator_select == "Prices") {
      labels <- sprintf("<strong>%s</strong><br/>%s IQD (%s)", departamento$Admin1Name, format(departamento[[map_indicator_select()]], big.mark=","), format(departamento$Date.x, "%b %Y")) %>% lapply(htmltools::HTML)
    } else {
      labels <- sprintf("<strong>%s</strong><br/>%s&#37; of assessed traders (%s)", departamento$Admin1Name, departamento[[map_indicator_select()]], format(departamento$Date.x, "%b %Y")) %>% lapply(htmltools::HTML)
    }
    ...
    leafletProxy("map") %>%
      ...
      execute_if(input$map_indicator_select == "Prices",
                 addPolygons(data = departamento, group = "Departamento", fill = TRUE, fillOpacity = 0.7, fillColor = ~pal(departamento[[map_indicator_select()]]),
      ...
      execute_if(input$map_indicator_select == "Prices",
                 addLegend("bottomright", pal = pal, values = departamento[[map_indicator_select()]],
                           title = "Price:",
                           labFormat = labelFormat(prefix = "COL "),
                           opacity = 1
                 )
...
>>>>>>> 7cbf0e16d42237998405f06fe3b8891c6026a20d
