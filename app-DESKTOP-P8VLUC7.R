             ...
             tabPanel("Data Explorer", 
                      ...
                      sidebarLayout(
                        sidebarPanel(
                          ...
                          # Cuando se selecciona Departamento
                          conditionalPanel(condition = "input.plot_aggregation == 'Departamento'",
                                           pickerInput("plot_by_departamento_item",
                                                       label = "Productos:",
                                                       choices = lapply(split(full_list$Item, full_list$Group), as.list),
                                                       selected = "Arroz (1 Kg)",
                                                       multiple = FALSE
...
table_datasetInput1 <- reactive({
  full %>% filter(
    is.null(input$`Lista de productos compilados`) | Departamento %in% input$`Lista de productos compilados`,
    Date >= input$Fecha[1] & Fecha <= input$Fecha[2]
  ) %>%
    select("Fecha", "Departamento", input$`Lista de productos alimentarios`$`Lista de productos no alimentarios`)
})

table_datasetInput <- reactive({
  if (input$table_aggregation == "Departamento") {table_datasetInput1()}
})
